...
class ExplicitRungeKutta:
    ...
    def __shu_osher_time_step(self, q_old, t_old, delta_t, rhs_function):
        ...
        for i in range(1,self.num_stages+1):
            stages.append(self.__shu_osher_stage(t_old, delta_t,
                rhs_function, stages, i))

        # q^{n+1} = y_s
        return stages[-1]

    # y_i = sum{j=0}{i-1}{a_{ij}y_j + delta_t*b_{ij}F(t_n + c_j delta_t, y_j)}
    def __shu_osher_stage(self, t_old, delta_t, rhs_function, stages,
            stage_num):
    ...
    def __butcher_time_step(self, q_old, t_old, delta_t, rhs_function):
        ...
        q_stages = []
        for i in range(self.num_stages):
            q_stages.append(self.__butcher_stage(q_old, t_old, delta_t,
                rhs_function, q_stages, i))
    ...
    def __butcher_stage(self, q_old, t_old, delta_t, rhs_function, q_stages,
            stage_num):
