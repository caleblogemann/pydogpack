...
class SSP2(LowStorageExplicitRungeKutta):
    ...
    def time_step(self, q_old, t_old, delta_t, rhs_function):
        ...
        for i in range(1,self.num_stages+1):
            ...
            q_new = (self.alpha[i,i-1]*q_new + 
                delta_t*self.beta[i,i-1]*rhs_function(time, q_new))
...
class SSP3(LowStorageExplicitRungeKutta):
    def __init__(self, n = 2):
        ...
        alpha[int(n*(n + 1.0)/2.0), int((n - 1.0)*(n - 2.0)/2.0)] = n/(2.0*n - 1.0)
    ...
    def time_step(self, q_old, t_old, delta_t, rhs_function):
        ...
        for i in range(1,first_interval+1):
            time = t_old + self.c[i]*delta_t
            q_new = self.alpha[i,i-1]*q_new + delta_t*self.beta[i,i-1]*rhs_function(time, q_new)
        y_tmp = np.copy(q_new)
