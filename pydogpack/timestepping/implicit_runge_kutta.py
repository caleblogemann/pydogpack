...
class DiagonallyImplicitRungeKutta:
    ...
    def time_step(self, q_old, t_old, delta_t, rhs_function, solve_function):
        if(self.isButcherForm):
            ...
            return self.__shu_osher_time_step(q_old, t_old, delta_t, 
                rhs_function, solve_function)
    
    def __butcher_time_step(self, q_old, t_old, delta_t, rhs_function, 
            solve_function):
        stages = []
        for i in range(self.num_stages):
            stages.append(self.__butcher_stage(q_old, t_old, delta_t,
                rhs_function, solve_function, stages, i))
    ...
    def __butcher_stage(self, q_old, t_old, delta_t, rhs_function, 
            solve_function, stages, stage_num):
    ...
    def __shu_osher_time_step(self, q_old, t_old, delta_t, rhs_function,
        ...
        for i in range(1, self.num_stages+1):
            stages.append(self.__shu_osher_stage(t_old, delta_t,
                rhs_function, solve_function, stages, i))

        # q^{n+1} = y_s
        return stages[-1]

    def __shu_osher_stage(self, t_old, delta_t, rhs_function, solve_function,
            stages, stage_num):
        ...
        for j in range(stage_num):
            ...
            if(self.b[stage_num, j] != 0.0):
                ...
                stage_rhs += (delta_t*self.b[stage_num, j]*
                    rhs_function(time, stages[j]))
        
        # solve equation
        time = t_old + self.c[stage_num]*delta_t
        stage_function = lambda y_i: ((1.0 - self.a[stage_num, stage_num])*y_i 
