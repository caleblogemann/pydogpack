...
class DerivativeRiemannSolver(riemann_solvers.RiemannSolver):
    ...
    def solve_dg_solution(self, dg_solution, face_index):
        ...
        derivative_left_state = basis_.evaluate_canonical(
            1.0, dg_solution.derivative, left_elem
        )
        derivative_right_state = basis_.evaluate_canonical(
            -1.0, dg_solution.derivative, right_elem
        )

        derivative_average = self.interface_average(
            derivative_left_state, derivative_right_state
        )
        derivative_jump = self.interface_jump(
            derivative_left_state, derivative_right_state
        )

        jump = self.interface_jump(left_state, right_state)

        return derivative_average + self.c11 * jump + self.c12 * derivative_jump
...
class DerivativeDirichlet(boundary.Dirichlet):
    ...
    def evaluate_boundary(self, dg_solution, face_index, riemann_solver):
        ...
        if left_elem_index != -1:
            ...
            interior_derivative_state = basis_.evaluate_canonical(
                1.0, dg_solution.derivative, left_elem_index
            )
        else:
            # left boundary
            normal = -1.0
            ...
            interior_derivative_state = basis_.evaluate_canonical(
                -1.0, dg_solution.derivative, right_elem_index
            )

        vertex = dg_solution.mesh.vertices[face_index]
        return (
            interior_derivative_state
            - self.c11 * (interior_state - self.boundary_function(vertex)) * normal
        )
